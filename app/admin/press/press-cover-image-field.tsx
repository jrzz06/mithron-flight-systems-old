...
export function PressCoverImageField({
  defaultSrc = "",
  defaultAlt = ""
}: {
  defaultSrc?: string;
  ...
  async function onFileChange(file: File | null) {
    ...
    try {
      ...
      formData.append("usage_scope", "press");
      ...
      if (!result.ok || !result.src) {
        ...
        setError(message);
        notify.error(message, { source: "cms", id: "press-cover:upload-error" });
        return;
      }
      setSrc(result.src);
      if (result.alt) setAlt(result.alt);
      notify.success(FEEDBACK_MESSAGES.imageUploaded, { source: "cms", id: "press-cover:upload" });
    } catch (err) {
      const message = err instanceof Error ? err.message : "Upload failed.";
      setError(message);
      notify.error(message, { source: "cms", id: "press-cover:upload-catch" });
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <div className="grid gap-3 rounded-[10px] border border-[var(--platform-border)] bg-[var(--platform-surface-muted)]/40 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-medium text-[var(--platform-text-primary)]">Cover image (16:9)</p>

...
begin
  ...
  if 'revision' = any(v_columns) then
    v_update_patch := v_update_patch || jsonb_build_object('revision', v_revision);
  end if;

  v_snapshot := v_before || v_update_patch || jsonb_build_object('revision', v_revision);

  raise log 'cms_mutate_content_with_revision revision request_id=% table=% entity_id=% next_revision=% patch_keys=%',
    p_request_id, v_entity_table, v_entity_id, v_revision, (

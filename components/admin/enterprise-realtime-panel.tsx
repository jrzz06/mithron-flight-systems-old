  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="type-badge font-bold uppercase tracking-[0.16em] text-[#7ce7c9]">Realtime</p>
          <h2 className="mt-1 text-sm font-semibold text-white/84">{config.label}</h2>
        </div>
        <span className="rounded-full border border-white/10 bg-white/[0.055] px-3 py-1 type-badge font-bold uppercase tracking-[0.14em] text-white/58">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
          <p className="type-badge uppercase tracking-[0.14em] text-white/34">Tables</p>
          <p className="mt-1 font-[var(--type-display)] text-xl font-semibold">{diagnostics.tables.length}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
          <p className="type-badge uppercase tracking-[0.14em] text-white/34">Events</p>
          <p className="mt-1 font-[var(--type-display)] text-xl font-semibold">{diagnostics.receivedEvents}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
          <p className="type-badge uppercase tracking-[0.14em] text-white/34">Reconnects</p>
      ...
      {!compact ? (
        <>
          <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.035] p-3">
            <p className="type-badge uppercase tracking-[0.14em] text-white/34">Channel</p>
            <p className="mt-1 break-all font-mono type-meta text-white/52">{diagnostics.channelName}</p>
          ...
          <div data-realtime-security-diagnostics className="mt-3 rounded-xl border border-white/10 bg-white/[0.035] p-3">
            <p className="type-badge uppercase tracking-[0.14em] text-white/34">Subscription security</p>
      ...
      <div className="mt-4 grid gap-2">
        {latestEvents.length ? latestEvents.map((event) => (
          <div key={`${event.table}-${event.eventType}-${event.commitTimestamp}-${JSON.stringify(event.record ?? event.oldRecord ?? {})}`} className="rounded-xl border border-white/10 bg-white/[0.035] px-3 py-2">
            ...
            <p className="mt-1 type-meta uppercase tracking-[0.12em] text-white/38">{event.eventType} {event.commitTimestamp ?? ""}</p>
          </div>
        )) : (
